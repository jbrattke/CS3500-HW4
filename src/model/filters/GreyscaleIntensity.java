...
public class GreyscaleIntensity extends GreyscaleFilter {
  /**
  ...
  public ImageModel apply(ImageModel image) {
    ...
    for (int i = 0; i < image.getHeight(); i++) {
      for (int j = 0; j < image.getWidth(); j++) {
        int channelAvg = (srcPixels[i][j].getRed() + srcPixels[i][j].getGreen()
                + srcPixels[i][j].getBlue()) / 3;
        newPixels[i][j] = new PixelRGB(channelAvg, channelAvg, channelAvg);
      }
    }
